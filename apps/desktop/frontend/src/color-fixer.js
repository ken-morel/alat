...
function startColorConversion() {
  const conversions = [];
  const oklchRegex = /oklch\(([^)]+)\)/g;

  // 1. Collect all required conversions
        ...
        if (rule.style) {
          for (const prop of Array.from(rule.style)) {
            ...
            if (value.includes("oklch(")) {
              conversions.push({ rule, prop, value });
            }
          }
        }
      }
  ...
  // 2. Process conversions with a delay
  let index = 0;
  function processNext() {
    if (index >= conversions.length) {
      // Optional: loop back to the start to handle dynamically added styles
      // index = 0;
      return;
    }

    const { rule, prop, value } = conversions[index];

    const newValue = value.replace(oklchRegex, (match, oklchValue) => {
      const parts = oklchValue
        .replace(/deg|%/g, "")
        .split(/\s+/)
        .filter(Boolean);

      const l = parseFloat(parts[0]) / 100; // Convert percentage to 0-1 range
      const c = parseFloat(parts[1]);
      let h = parseFloat(parts[2]);

      const [r, g, b] = oklch2rgb([l, c, h]).map((c) =>
    ...
    index++;
    setTimeout(processNext, 50); // 50ms delay between each conversion
  }
