  ...
  <div class="flex items-center justify-center group cursor-pointer">
    <div class="relative">
      ...
      {#if sendingPercent < 100}
        <div
          class="absolute -top-1 -right-1 w-3 h-3 bg-success-500 rounded-full animate-pulse shadow-lg"
        ></div>
      {/if}
    </div>
  </div>
...
{#if isOpen}
  <div
    {...popover.content}
    in:scale={{ opacity: 0, duration: 300, start: 0.95 }}
    class="bg-surface-50-900 border border-surface-300-700 rounded-xl shadow-2xl w-[520px] h-[450px] overflow-hidden backdrop-blur-sm z-50"
  >
    ...
    <div class="flex flex-col h-full">
      {#if status}
        ...
        <div
          ...
          {...tabs.triggerList}
        >
          <div class="flex bg-surface-200-800 rounded-lg p-1 relative">
            <button
              class="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 relative z-10"
...
{#snippet transferTab(devices: filesend.FileTransfersStatusDevice[])}
  {#each devices as device, i (device.Device.ID)}
    <div
      class="bg-surface-100-900 border border-surface-300-700 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-200 hover:border-primary-500/30"
